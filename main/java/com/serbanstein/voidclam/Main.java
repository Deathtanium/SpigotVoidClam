...
public class Main extends JavaPlugin {
	...
	public void onEnable() {
		...
		syncTasks.add(new BukkitRunnable() {
			...
			}}.runTaskTimer(Main.getPlugin(Main.class), 0, 3*20));
			...
			public void run() {
				...
				for (i = 1; i <= moduleNumber; i++){
					...
					if (modules[i].energy > 4 * modules[i].currentSize && modules[i].currentSize < 15) {
						...
						if (cst > 2 * csize) hasRoom = 0;
						if (hasRoom == 1 || modules[i].energy > 10 * modules[i].currentSize) {
							if (hasRoom == 1) modules[i].energy -= 2 * modules[i].currentSize;
							else modules[i].energy -= 5 * modules[i].currentSize;
		...
		}.runTaskTimer(Main.getPlugin(Main.class), 0, 300*20));
		...
		//check if blacklisted block has become available
		syncTasks.add(new BukkitRunnable() {
			public void run() {
				for(int i=1;i<=moduleNumber;i++) modules[i].lightsBlackList.clear();
			}}.runTaskTimer(Main.getPlugin(Main.class), 0, 60*60*20));

		syncTasks.add(new BukkitRunnable(){
			public void run(){
				for(int i=1;i<=moduleNumber;i++)
				{
					if(Objects.requireNonNull(getWorld(worldName)).getBlockAt(modules[i].x,modules[i].y,modules[i].z).getType() !=Material.NETHER_WART_BLOCK){
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (command.getName().equalsIgnoreCase("voidclamkill") && sender.hasPermission("debugVoidClam")) {
			...
			int tno;
			try{tno = Integer.parseInt(args[0]);}catch(NumberFormatException e){return false;}
			if (modules[tno] == null || tno > moduleNumber || moduleNumber == 0) {
				sender.sendMessage("Bad number");
				return true;
			}
