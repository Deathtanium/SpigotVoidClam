...
public class Main extends JavaPlugin {
	...
	public void landsToolkit(Plugin myPlugin){
		landsIntegration = new LandsIntegration(myPlugin);
	}
	...
	public void onLoad() {
		makeLists();
		getWorldName();
		landsToolkit(getPlugin(Main.class));
	...
	public void onEnable() {
		...
		syncTasks.add(new BukkitRunnable() {
			...
			}}.runTaskTimer(Main.getPlugin(Main.class), 0, 30*20));
			...
			public void run() {
				...
				for (i = 1;i <= moduleNumber; i++){
					...
					if (modules[i].energy > 4 * modules[i].currentSize && modules[i].currentSize < 15) {
						hasRoom = 1;
						Area 	area1=landsIntegration.getAreaByLoc(new Location(getWorld(worldName),x+csize+2,0,z+csize+2)),
								area2=landsIntegration.getAreaByLoc(new Location(getWorld(worldName),x-csize-2,0,z+csize+2)),
								area3=landsIntegration.getAreaByLoc(new Location(getWorld(worldName),x+csize+2,0,z-csize-2)),
								area4=landsIntegration.getAreaByLoc(new Location(getWorld(worldName),x-csize-2,0,z-csize-2)),
								area5=landsIntegration.getAreaByLoc(new Location(getWorld(worldName),x+csize+2,0,z)),
								area6=landsIntegration.getAreaByLoc(new Location(getWorld(worldName),x-csize-2,0,z)),
								area7=landsIntegration.getAreaByLoc(new Location(getWorld(worldName),x,0,z-csize-2)),
								area8=landsIntegration.getAreaByLoc(new Location(getWorld(worldName),x,0,z-csize-2));
						if(area1!=null){
							if(!area1.hasFlag(Flags.ENTITY_GRIEFING)){
								hasRoom = 0;
							}
						}
						if(area2!=null){
							if(!area2.hasFlag(Flags.ENTITY_GRIEFING)){
								hasRoom = 0;
							}
						}
						if(area3!=null){
							if(!area3.hasFlag(Flags.ENTITY_GRIEFING)){
								hasRoom = 0;
							}
						}
						if(area4!=null){
							if(!area4.hasFlag(Flags.ENTITY_GRIEFING)){
								hasRoom = 0;
							}
						}
						if(area5!=null){
							if(!area5.hasFlag(Flags.ENTITY_GRIEFING)){
								hasRoom = 0;
							}
						}
						if(area6!=null){
							if(!area6.hasFlag(Flags.ENTITY_GRIEFING)){
								hasRoom = 0;
							}
						}
						if(area7!=null){
							if(!area7.hasFlag(Flags.ENTITY_GRIEFING)){
								hasRoom = 0;
							}
						}
						if(area8!=null){
							if(!area8.hasFlag(Flags.ENTITY_GRIEFING)){
								hasRoom = 0;
							}
						}
						cst = 0;
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (command.getName().equalsIgnoreCase("voidclamgrow") && (sender.hasPermission("debugVoidClam"))) {
			if(args.length!=1) return false;
			int tno;
			try{tno = Integer.parseInt(args[1]);}catch(NumberFormatException e){return false;}
			if(tno > moduleNumber) {
				sender.sendMessage("Bad number");
				return true;
			}
