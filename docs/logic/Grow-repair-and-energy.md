...
Completion still goes through **`tickGrowPendingCheck`** with **`growCommandTargetSize == -1`**, which runs **`runAutoGrowRoutineSingle`** for that clam only (repair to `currentSize`, optional +1 grow using the same heuristics as before).
...
1. If **shell damage** (`inspectObsidianShellDamage.shellMissing() > 0`): repair-sized `clamReSize`, set `prioritizeRepairOreSeek` from material vs damage, return — **no** change to `materialSeekThreshold`.
...
4. If `energy <= clam_grow_energymultiplier * currentSize` or `currentSize >= clam_size_max`, skip growth.
5. Else scan interior volume for blast resistance / “has room” heuristics (`cst > 10 * cSize` fails).
6. If room and `clam_grow_material_cost > 0` but `material` is below that cost, skip growth (energy is not zeroed).
7. If room: optionally debit `clam_grow_material_cost`, zero energy, `clamReSize` to a larger size per config/heuristics, updating `currentSize`.
...
- **Lose 1** energy when stamina would fail on a dig step (including last-gasp breaks and unbreakable sentinel costs); see [[Pathfinding-and-reach]].
